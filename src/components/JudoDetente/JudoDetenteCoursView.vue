...
async function fetchCours() {
  try {
    ...
    console.log("👉 Tous les cours reçus de l'API :", coursAllJudoDetente);
    ...
    if (foundcoursJudoDetente && foundcoursJudoDetente.length > 0) {
      ...
      errorMessage.value = "Aucun cours de JudoDetente trouvé.";
    }

    console.log("Cours chargés", coursJudoDetente.value);
  } catch (error) {
...
<template>
  <div class="container-fluid p-0 bg-dark text-light max-vh-100">
    <h2 class="mb-5 text-center">NOS COURS DE JUDO DETENTE</h2>
    ...
    <div v-else class="container-fluid cours-quinconce-container">
      ...
      <div v-for="(cours, index) in coursJudoDetente" :key="cours.coursId" class="row align-items-center course-row">
