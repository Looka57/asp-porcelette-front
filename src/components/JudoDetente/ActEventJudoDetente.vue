...
const JUDODETENTE_DISCIPLINE_ID = 4
...
async function fetchEvenement() {
  try {
    ...
    const evenementsFiltres = evenementsAPI.filter(e =>
      e.disciplineId === JUDODETENTE_DISCIPLINE_ID &&
      new Date(e.dateFin) >= maintenant
    )

    if (evenementsFiltres.length === 0) {
      errorMessage.value = "Aucun événement de Judo Détente à venir trouvé."
      evenements.value = []
      return
    }
      ...
      <div class="row g-5">
        ...
        <div class="col-lg-6 news-section">
          <div class="text-center mb-5">
            <span class="text-uppercase tracking-wider fs-7 fw-bold text-warning d-block mb-2">Informations</span>
          ...
          <div v-else-if="errorMessageActu"
            class="alert alert-danger bg-warning bg-opacity-25 text-warning border-0 text-center py-4">
            {{ errorMessageActu }}
          </div>
              ...
              <div class="card event-card h-100 shadow-lg border border-secondary border-opacity-10 rounded-4 p-3">
                <div class="card-body d-flex flex-column">
                  ...
                  <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="date-text text-warning fw-semibold fs-6">
                  ...
                  <router-link :to="`/actualite/${actu.actualiteId}`"
                    class="btn btn-warning-custom fw-bold rounded-pill mt-auto align-self-start px-4 py-2">
                    Lire l'actualité
                  </router-link>
        ...
        <div class="col-lg-6 events-section border-lg-start border-secondary border-opacity-10 ps-lg-5">
          <div class="text-center mb-5">
            ...
            <h2 class="display-5 text-white fw-bold m-0">Événements Judo Détente</h2>
          </div>
...
.event-card:hover {
  ...
  border-color: rgba(231, 210, 92, 0.3) !important;
}
...
/* Bouton personnalisé couleur or/jaune */
.btn-warning-custom {
  background-color: transparent;
  border: 2px solid #e7d25c;
  color: #e7d25c;
...
.btn-warning-custom:hover {
  ...
  color: #1a1d21;
  box-shadow: 0 4px 12px rgba(231, 210, 92, 0.3);
}
