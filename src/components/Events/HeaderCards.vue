...
<template>
  ...
  <div class="headerLink w-100 d-flex align-items-center my-4 p-3 rounded">
    ...
    <button class="btn btn-outline-warning d-flex justify-content-center align-items-center mr-5" type="button"
    ...
    <ul class="nav mb-0">
      <li class="nav-item"
        v-for="filter in ['Tous', 'À venir', 'Judo', 'Judo Détente', 'Aïkido', 'Jujitsu', 'Archives']" :key="filter">
        <a class="nav-link" :class="{ active: activeFiltre === filter }" href="#" @click.prevent="selectFilter(filter)">
          {{ filter }}
        </a>
      </li>
    </ul>
