...
async function fetchEvenement() {
  try {
    ...
    const evenementsFiltres = evenementsAPI.filter(e =>
      e.disciplineId === AIKIDO_DISCIPLINE_ID &&
      new Date(e.dateFin) >= maintenant
    )

    if (evenementsFiltres.length === 0) {
      errorMessage.value = "Aucun événement d'Aïkido à venir trouvé."
      evenements.value = []
      return
    }
      ...
      <div class="row g-5">
        ...
        <div class="col-lg-6 news-section">
          <div class="text-center mb-5">
            <span class="text-uppercase tracking-wider fs-7 fw-bold text-info-custom d-block mb-2">Informations</span>
          ...
          <div v-else-if="errorMessageActu" class="alert alert-danger bg-info bg-opacity-25 text-info border-0 text-center py-4">
            {{ errorMessageActu }}
          </div>
              ...
              <div class="card event-card h-100 shadow-lg border border-secondary border-opacity-10 rounded-4 p-3">
                <div class="card-body d-flex flex-column">
                  ...
                  <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="date-text text-info-custom fw-semibold fs-6">
                  ...
                  <router-link :to="`/actualite/${actu.actualiteId}`" class="btn btn-info-custom fw-bold rounded-pill mt-auto align-self-start px-4 py-2">
                    Lire l'actualité
                  </router-link>
        ...
        <div class="col-lg-6 events-section border-lg-start border-secondary border-opacity-10 ps-lg-5">
          <div class="text-center mb-5">
            <span class="text-uppercase tracking-wider fs-7 fw-bold text-info-custom d-block mb-2">Agenda</span>
            <h2 class="display-5 text-white fw-bold m-0">Événements Aïkido</h2>
          </div>
          ...
          <div v-else-if="errorMessage" class="alert alert-danger bg-info bg-opacity-25 text-info border-0 text-center py-4">
            {{ errorMessage }}
          </div>
              ...
              <div class="card event-card h-100 shadow-lg border border-secondary border-opacity-10 rounded-4 p-3">
                <div class="card-body d-flex flex-column">
                  ...
                  <router-link :to="`/evenement/${evenement.evenementId}`"
                    class="btn btn-info-custom fw-bold rounded-pill mt-auto align-self-start px-4 py-2">
                    En savoir plus
                  </router-link>
...
.event-card:hover {
  ...
  border-color: rgba(49, 179, 208, 0.3) !important;
}
...
/* Bouton personnalisé couleur bleue Aïkido */
.btn-info-custom {
  background-color: transparent;
  border: 2px solid #31b3d0;
  color: #31b3d0;
...
.btn-info-custom:hover {
  ...
  color: #1a1d21;
  box-shadow: 0 4px 12px rgba(49, 179, 208, 0.3);
}
