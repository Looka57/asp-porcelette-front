...
async function fetchEvenement() {
  try {
    ...
    if (evenementsFiltres.length === 0) {
      errorMessage.value = "Aucun événement d'Aikido à venir trouvé.";
      evenements.value = [];
      return;
    }
  ...
  <div class="container-fluid mt-5 bg-dark text-light min-vh-100 p-4">
    <div class="row">
            ...
            <div class="card event-card h-100 shadow-lg">
              <div class="card-body">
                ...
                <img :src="actu.imageUrl
                  ? actu.imageUrl // Utilise l'URL brute (si elle est relative, elle sera demandée à la racine du domaine)
                  : '/images/actualites/placeholder-styling.jpg'" alt="Événement" class="news-image mb-3">

                <router-link :to="`/actualite/${actu.actualiteId}`" class="btn btn-info fw-bold">
                  Lire l'actualité
                </router-link>
              </div>
            </div>
      ...
      <div class="col-lg-6 col-md-6 col-sm-12 p-3 events-section border-start border-secondary">
        <h2 class="text-center text-warning mb-5 display-5">Évènements de l'Aikido à Venir</h2>
        ...
        <div v-else-if="evenements.length === 0" class="text-center text-secondary p-4">
          Aucun événement de l'Aikido à venir n'est planifié. Restez à l'affût !
        </div>
            ...
            <div class="card event-card h-100 shadow-lg">
              <div class="card-body">
                ...
                <router-link :to="`/evenement/${evenement.evenementId}`"
                  class="btn btn-info fw-bold mt-auto stretched-link">
                  Lire l'actualité
                </router-link>
              </div>
            </div>
